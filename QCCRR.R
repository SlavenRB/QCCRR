...
podaci <- read.spss ("D:\\za_sebe\\phd_studije\\multivarijacione_metode\\KanonickaDiskriminacionaAnaliza\\ASTMA.SAV", use.value.labels = F)

x <- data.frame(podaci$mmpil, podaci$mmpif, podaci$mmpik, podaci$mmpipa)
y <- data.frame(podaci$mmpid, podaci$mmpihy, podaci$mmpipd)
inc <- 0.05

x <- data.frame(podaci$mmpil, podaci$mmpif, podaci$mmpik, podaci$mmpipa)
y <- data.frame(podaci$mmpid, podaci$mmpihy, podaci$mmpipd, podaci$mmpipt)
inc <- 0.05

x <- data.frame(podaci$mmpil, podaci$mmpif, podaci$mmpik, podaci$mmpipa)
y <- data.frame(podaci$mmpid, podaci$mmpihy, podaci$mmpipd, podaci$mmpipt, podaci$mmpima)
inc <- 0.05

x <- data.frame(podaci$mmpil, podaci$mmpif, podaci$mmpik)
y <- data.frame(podaci$mmpid, podaci$mmpihy)
inc <- 0.05
...
podaci <- read.spss ("D:\\za_sebe\\phd_studije\\multivarijacione_metode\\KanonickaKorelacionaAnaliza\\licnost_i_inteligencija.sav", use.value.labels = F)

x <- data.frame(podaci$neurot, podaci$ekstra, podaci$otvor, podaci$dobrodus, podaci$savest)
y <- data.frame(podaci$al_4, podaci$alf_7, podaci$g_sn, podaci$it_1, podaci$cf_2, podaci$gt_7, podaci$it_2, podaci$s_1, podaci$d_48)
inc <- 0.05
